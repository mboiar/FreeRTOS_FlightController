...
static StreamBufferHandle_t crsfStream;
    ...
    if (n==0) {
      ...
      for (size_t i = 0; i<n; i++) {
            if (radio_parse_crsf_byte(&frame, rx_buf[i], &crsf_state)) {
              ...
              uint8_t len = 2;
              len += snprintf(RadioRXLog+len, sizeof(RadioRXLog)-len, "%d %d %d ", frame.addr, frame.len, frame.type);
              switch (frame.type) {
              case CRSF_TYPE_RC:
                radio_unpack_rc(&rc_data, frame.payload);
                for (size_t i = 0; i < 16; i++) {
                  len += snprintf(RadioRXLog+len, sizeof(RadioRXLog)-len, "%d ", rc_data.ch_data[i]);
                }
                break;
              default:
                for (size_t i = 0; i < frame.len-2; i++) {
                  len += snprintf(RadioRXLog+len, sizeof(RadioRXLog)-len, "%d ", frame.payload[i]);
                }
                break;
              }
              RadioRXLog[0] = len;
...
void HAL_UART_RxHalfCpltCallback(UART_HandleTypeDef* huart) {
  if (huart == &huart2) {
    radio_dma_to_buffer(DMA_RX_LEN/2);
  }
}
