...
HAL_StatusTypeDef w25q64_sector_erase(uint32_t addr,
  ...
  uint8_t instr[4] = {option, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF,
                      (addr) & 0xFF};
...
HAL_StatusTypeDef w25q64_isbusy(bool *busy) {
  ...
  HAL_StatusTypeDef status =
      HAL_SPI_TransmitReceive(hspi, instr, instr, 2, MAX_SPI_DELAY);
  w25q64_setCS(GPIO_PIN_SET);
  if (status == HAL_OK) {
    *busy = instr[1] & 0x01;
  }
...
void w25q64_transfer_done() { w25q64_setCS(GPIO_PIN_SET); }
...
HAL_StatusTypeDef w25q64_device_info(device_info *data) {
  ...
  uint8_t data_rx[6] = {0};
  HAL_StatusTypeDef status =
      HAL_SPI_TransmitReceive(&hspi1, instr, data_rx, 6, MAX_SPI_DELAY);
  w25q64_setCS(GPIO_PIN_SET);
...
HAL_StatusTypeDef w25q64_read_data(uint8_t *data_rxtx, uint16_t size,
  ...
  w25q64_setCS(GPIO_PIN_RESET);
  HAL_StatusTypeDef status =
      HAL_SPI_TransmitReceive(hspi, data_rxtx, data_rxtx, size, HAL_MAX_DELAY);
  w25q64_setCS(GPIO_PIN_SET);
  return status;
}
...
HAL_StatusTypeDef w25q64_page_program_DMA(uint8_t *data_rx, uint16_t size,
                                          uint32_t addr) {
...
HAL_StatusTypeDef w25q64_page_program_IT(uint8_t *data_rx, uint16_t size,
                                         uint32_t addr) {
  data_rx[0] = w25q64_PAGE_PROGRAM;
  data_rx[1] = (addr >> 16) & 0xFF;
  data_rx[2] = (addr >> 8) & 0xFF;
...
HAL_StatusTypeDef w25q64_status(status_registers *status_bits) {
  uint8_t txrx[6] = {w25q64_READ_STATUS1, 0, w25q64_READ_STATUS2, 0,
                     w25q64_READ_STATUS3, 0};
  w25q64_setCS(GPIO_PIN_RESET);
  HAL_StatusTypeDef status =
      HAL_SPI_TransmitReceive(hspi, txrx, txrx, 6, MAX_SPI_DELAY);
  w25q64_setCS(GPIO_PIN_SET);
