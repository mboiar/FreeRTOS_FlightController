...
/**
  * @brief  Program up to 256 (1 page) previously erased (0xFF) registers in DMA mode. BUSY = 1 while operation in progress.
  ...
  */
HAL_StatusTypeDef w25q64_page_program_DMA(uint8_t* data, uint16_t size, uint32_t addr);


/**
  * @brief  Program up to 256 (1 page) previously erased (0xFF) registers in interrupt mode. BUSY = 1 while operation in progress.
  * @param  data  tx buffer
  * @param  size  size of data in bytes
  * @param  addr  address of first register to write to
  * @retval HAL_StatusTypeDef
  * @warning If data exceeds page size, wrap to the first address of the page.
  */
HAL_StatusTypeDef w25q64_page_program_IT(uint8_t* data, uint16_t size, uint32_t addr);
...
/**
  ...
  *         
  *         Unlimited amount of data can be read. First 4 bytes contain instruction.
...
/**
  * @brief  (NOT IMPLEMENTED) Write (up to 64 MB of data). BUSY = 1 while operation in progress.
  * @param  data  tx buffer
  * @param  size  size of data in bytes
  * @param  addr  address of first register to write to
  * @retval HAL_StatusTypeDef
